...
var t5 = class {
  constructor(t6, r3, s9) {
    this._createTexture = t6, this._parametersKey = r3, this._repository = /* @__PURE__ */ new Map(), this._orphanCache = s9.newCache(`procedural-texture-repository:${e()}`, (e7) => e7.dispose());
  }
  destroy() {
    for (const { texture: e7 } of this._repository.values()) e7.dispose();
    this._repository.clear(), this._orphanCache.destroy();
  }
  swap(e7, t6 = null) {
    const r3 = this._acquire(e7);
    return this.release(t6), r3;
  }
  release(e7) {
    if (null == e7) return;
    const t6 = this._parametersKey(e7), r3 = this._repository.get(t6);
    if (r3 && (r3.refCount--, 0 === r3.refCount)) {
      this._repository.delete(t6);
      const { texture: e8 } = r3;
      this._orphanCache.put(t6, e8);
    }
  }
  _acquire(e7) {
    if (null == e7) return null;
    const t6 = this._parametersKey(e7), s9 = this._repository.get(t6);
    if (s9) return s9.refCount++, s9.texture;
    const o5 = this._orphanCache.pop(t6) ?? this._createTexture(e7), i3 = new r2(o5);
    return this._repository.set(t6, i3), o5;
  }
};
var r2 = class {
  constructor(e7) {
    this.texture = e7, this.refCount = 1;
  }
};
...
function u(t6, u3) {
  return new t5((e7) => {
    const { encodedData: u4, textureSize: c4 } = a2(e7), f4 = new p2();
    return f4.internalFormat = G.RGBA, f4.width = c4, f4.height = 1, f4.wrapMode = D.REPEAT, new w2(t6, f4, u4);
  }, (t7) => `${t7.pattern.join(",")}-r${t7.pixelRatio}`, u3);
}
function a2(e7) {
  const r3 = c2(e7), o5 = 1 / e7.pixelRatio, n5 = f2(e7), i3 = s6(e7), u3 = (Math.floor(0.5 * (i3 - 1)) + 0.5) * o5, a3 = [];
  let l = 1;
  for (const t6 of r3) {
    for (let e8 = 0; e8 < t6; e8++) {
      const r4 = l * (Math.min(e8, t6 - 1 - e8) + 0.5) * o5 / u3 * 0.5 + 0.5;
      a3.push(r4);
    }
    l = -l;
  }
  const p5 = Math.round(r3[0] / 2), m2 = [...a3.slice(p5), ...a3.slice(0, p5)], d5 = new Uint8Array(4 * n5);
  let x = 0;
  for (const c4 of m2) o(c4, d5, x), x += 4;
  return { encodedData: d5, textureSize: n5 };
}
function c2(t6) {
...
vec4 blendStipple(vec4 color, float stippleAlpha) { return color; }`);
  ...
  h.include(t2), d5.draped || (d(g, d5), g.uniforms.add(new e3("worldToScreenPerDistanceRatio", ({ camera: e7 }) => 1 / e7.perScreenPixelRatio)).code.add(n2`float computeWorldToScreenRatio(vec3 segmentCenter) {
...
}`), h.uniforms.add(new s4("stipplePatternTexture", (e7) => e7.stippleTexture), new s3("stipplePatternSDFNormalizer", (e7) => m(e7.stipplePattern)), new s3("stipplePatternPixelSizeInv", (e7) => 1 / S(e7))), d5.stippleOffColorEnabled && h.uniforms.add(new e4("stippleOffColor", (e7) => s7(e7.stippleOffColor))), h.code.add(n2`float getStippleSDF(out bool isClamped) {
...
function C(C2) {
  ...
  F2.include(t4), F2.include(c, C2), F2.include(f3, C2), F2.include(e5, C2), F2.include(d3, C2);
  const k = T && !V;
  k && (z.uniforms.add(new s3("markerScale", (e7) => e7.markerScale)), F2.include(n4, { space: s5.World })), f(z, C2), z.uniforms.add(new t3("inverseProjectionMatrix", (e7) => e7.camera.inverseProjectionMatrix), new o2("nearFar", (e7) => e7.camera.nearFar), new s3("miterLimit", (e7) => "miter" !== e7.join ? 0 : e7.miterLimit), new o4("viewport", (e7) => e7.camera.fullViewport)), z.constants.add("LARGE_HALF_FLOAT", "float", 65500), P.add(e2.POSITION, "vec3"), P.add(e2.PREVPOSITION, "vec3"), P.add(e2.NEXTPOSITION, "vec3"), P.add(e2.SUBDIVISIONFACTOR, "float"), P.add(e2.UV0, "vec2"), R.add("vColor", "vec4"), R.add("vpos", "vec3"), R.add("vLineDistance", "float"), R.add("vLineWidth", "float");
  const B = E;
...
vReverseSegmentSDF = (isJoin && !isStartVertex) ? LARGE_HALF_FLOAT : (dot(pos.xy - segmentEnd, -segmentDir) * pos.w);`), E && (V ? z.uniforms.add(new e3("worldToScreenRatio", (e7) => 1 / e7.screenToPCSRatio)) : z.main.add(n2`vec3 segmentCenter = mix((nextPosition + position) * 0.5, (position + prevPosition) * 0.5, isEndVertex);
...
vec2(vStippleDistanceLimits.x, 1e34);`)), z.main.add(n2`
    ...
    }`), F2.fragment.include(p, C2), F2.include(p3, C2), A.include(e6), A.main.add(n2`discardBySlice(vpos);
discardByTerrainDepth();`), M ? A.main.add(n2`vec4 finalColor = vec4(1.0, 0.0, 1.0, 1.0);`) : (U && A.main.add(n2`
      ...
      `) : A.main.add(n2`float stippleAlpha = getStippleAlpha();`), O !== n3.ObjectAndLayerIdColor && A.main.add(n2`discardByStippleAlpha(stippleAlpha, ${n2.float(o3)});`), A.uniforms.add(new e4("intrinsicColor", (e7) => e7.color)), A.main.add(n2`vec4 color = intrinsicColor * vColor;`), _ && (A.uniforms.add(new e4("innerColor", (e7) => e7.innerColor ?? e7.color), new s3("innerWidth", (e7, i3) => e7.innerWidth * i3.camera.pixelRatio)), A.main.add(n2`float distToInner = abs(vLineDistance * gl_FragCoord.w) - innerWidth;
...
//# sourceMappingURL=chunk-R5M3BLNL.js.map
