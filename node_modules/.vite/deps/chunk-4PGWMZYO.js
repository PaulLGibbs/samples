import {
  _
} from "./chunk-IKTP3VKX.js";
import {
  s
} from "./chunk-K5XUX3RQ.js";
import {
  b,
  m as m2
...
var t = class {
  ...
  _pruneAroundCenterTile(t2, i2, r4, s2) {
    ...
    for (l2.sort((t3, r5) => (d2[0] = n2.x + o2 * (t3.col + 0.5), d2[1] = n2.y - _2 * (t3.row + 0.5), c2[0] = n2.x + o2 * (r5.col + 0.5), c2[1] = n2.y - _2 * (r5.row + 0.5), b(d2, i2) - b(c2, i2))); l2.length > 0; ) {
      const e2 = l2.pop();
...
var r3 = class {
  ...
  update(e2) {
    var _a, _b;
    const { resampling: i2, tileIndex: t2 } = this, { scale: l2, center: r4, resolution: n2 } = e2.state, { minScale: f, maxScale: d2 } = this.tileInfoView, u3 = !e2.stationary && l2 > this._previousScale;
    if (this._previousScale = l2, !i2 && (l2 > f || l2 < d2)) return this.tiles.length = 0, void this.clear();
    ...
    const { spans: y, lodInfo: g2 } = p2, { level: I } = g2;
    ...
    if (y.length > 0) for (const { row: s2, colFrom: h3, colTo: c2 } of y) for (let e3 = h3; e3 <= c2; e3++) {
      v++;
      const i3 = o.set(I, s2, g2.normalizeCol(e3), g2.getWorldForColumn(e3)).id;
      let l3 = t2.get(i3);
  ...
  _hasReadyAncestor(t2, s2) {
    ...
    for (const o2 of this.tileIndex.values()) if (o2.isReady && o2.key.level >= s2 && o2.key.level < t2.level) {
      const t3 = u2();
      if (this.tileInfoView.getTileBounds(t3, o2.key, true), d(t3, l2)) return true;
    }
    return false;
  }
};
...
//# sourceMappingURL=chunk-4PGWMZYO.js.map
