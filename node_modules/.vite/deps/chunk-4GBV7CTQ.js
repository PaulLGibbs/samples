import {
  t as t2
} from "./chunk-E65O6Q2G.js";
import {
  u as u3
} from "./chunk-G7K556XL.js";
import {
  t
...
var T = class {
  async queryAll(e, i, n2) {
    if (!(e = (n2 == null ? void 0 : n2.ignoreInvisibleLayers) ? e.filter((e2) => e2.visible) : e.slice()).length) throw new s("elevation-query:invalid-layer", "Elevation queries require at least one elevation layer to fetch tiles from");
    const o2 = w.fromGeometry(i);
    let l = false;
    (n2 == null ? void 0 : n2.returnSampleInfo) || (l = true);
    const s2 = { ...R, ...n2, returnSampleInfo: true }, a2 = await this.query(e[e.length - 1], o2, s2), r = await this._queryAllContinue(e, a2, s2);
  ...
  async query(e, i, n2) {
    ...
    const o2 = { ...R, ...n2 }, l = new x(e, i.spatialReference, o2), s2 = o2.signal;
    return await e.load({ signal: s2 }), await q(l, i, s2), await this._selectTiles(l, s2), await F(l, s2), j2(l), b(l, s2);
  }
  ...
  async createSamplerAll(e, i, n2) {
    if (!(e = (n2 == null ? void 0 : n2.ignoreInvisibleLayers) ? e.filter((e2) => e2.visible) : e.slice()).length) throw new s("elevation-query:invalid-layer", "Elevation queries require at least one elevation layer to fetch tiles from");
    if (!i || "extent" !== i.type) throw new s("elevation-query:invalid-extent", "Invalid or undefined extent");
    const o2 = { ...R, ...n2, returnSampleInfo: true }, l = await this._createSampler(e[e.length - 1], i, o2);
    return this._createSamplerAllContinue(e, i, l, o2);
  }
  async _createSampler(e, t3, i, n2) {
    const o2 = i.signal;
    await e.load({ signal: o2 });
    const l = t3.spatialReference, s2 = e.tileInfo.spatialReference;
    l.equals(s2) || (await Q2([{ source: l, dest: s2 }], { signal: o2 }), t3 = O(t3, s2));
    const a2 = new _2(e, t3, i, n2);
    return await this._selectTiles(a2, o2), await F(a2, o2), new u3(a2.elevationTiles, a2.layer.tileInfo, a2.options.noDataValue);
  }
  async _createSamplerAllContinue(e, t3, i, n2) {
    if (e.pop(), !e.length) return i;
    const o2 = i.samplers.filter((e2) => !e2.tile.hasNoDataValues).map((e2) => o(e2.extent)), l = await this._createSampler(e[e.length - 1], t3, n2, o2);
  ...
  async _queryAllContinue(e, t3, n2) {
    ...
    const c = t3.geometry.clone(a2), u4 = await this.query(e[e.length - 1], c, n2), h2 = u4.sampleInfo;
  ...
  async _selectTiles(e, i) {
    ...
    else if ("finest-contiguous" === n2) await this._selectTilesFinestContiguous(e, i);
    else {
  ...
  async _selectTilesFinestContiguous(e, t3) {
    const { tileInfo: i, tilemapCache: n2 } = e.layer, o2 = E(i, n2, e.options.minDemResolution);
    await this._selectTilesFinestContiguousAt(e, o2, t3);
  }
  async _selectTilesFinestContiguousAt(e, i, l) {
    ...
    try {
      if (a2 && !L(a2)) await h(Promise.all(r.map((e2) => a2.fetchAvailability(e2.level, e2.row, e2.col, { signal: l }))), l);
      else if (await F(e, l), !e.allElevationTilesFetched()) throw e.clearElevationTiles(), new s("elevation-query:has-unavailable-tiles");
    } catch (c) {
      a(c), await this._selectTilesFinestContiguousAt(e, i - 1, l);
    }
  }
  async _selectTilesAuto(t3, i) {
    M(t3), S(t3);
    const l = t3.layer.tilemapCache;
    if (!l || L(l)) return this._selectTilesAutoPrefetchUpsample(t3, i);
    const s2 = t3.getTilesToFetch(), a2 = {}, r = s2.map(async (t4) => {
      const n2 = new t(null, 0, 0, 0, u2()), s3 = await _(l.fetchAvailabilityUpsample(t4.level, t4.row, t4.col, n2, { signal: i }));
      false !== s3.ok ? null != t4.id && (a2[t4.id] = n2) : a(s3.error);
    });
    await h(Promise.all(r), i), t3.remapTiles(a2);
  }
  async _selectTilesAutoPrefetchUpsample(e, t3) {
    const i = e.layer.tileInfo;
    await F(e, t3);
    let n2 = false;
    e.forEachTileToFetch((e2, t4) => {
      i.upsampleTile(e2) ? n2 = true : t4();
    }), n2 && await this._selectTilesAutoPrefetchUpsample(e, t3);
  }
};
var w = class _w {
  export() {
  ...
  async project(e, t3) {
    ...
    await Q2([{ source: this.spatialReference, dest: e }], { signal: t3 });
    const i = new u({ spatialReference: this.spatialReference, points: this.coordinates.map((e2) => [e2.x, e2.y]) }), n2 = O(i, e);
    if (!n2) return null;
  ...
  static fromGeometry(e) {
    ...
    if (t3.geometry = e, t3.spatialReference = e.spatialReference, e instanceof _w) t3.coordinates = e.coordinates.map((e2) => e2.clone()), t3._exporter = (t4, i) => {
      ...
      case "point": {
        ...
        t3.coordinates = n2 && o2 ? [new v(i.x, i.y, i.z, i.m)] : n2 ? [new v(i.x, i.y, i.z)] : o2 ? [new v(i.x, i.y, null, i.m)] : [new v(i.x, i.y)], t3._exporter = (t4, i2) => e.hasM ? new j(t4[0].x, t4[0].y, t4[0].z, t4[0].m, i2) : new j(t4[0].x, t4[0].y, t4[0].z, i2);
        break;
      }
      case "multipoint": {
        const i = e, { hasZ: n2, hasM: o2 } = i;
        t3.coordinates = n2 && o2 ? i.points.map((e2) => new v(e2[0], e2[1], e2[2], e2[3])) : n2 ? i.points.map((e2) => new v(e2[0], e2[1], e2[2])) : o2 ? i.points.map((e2) => new v(e2[0], e2[1], null, e2[2])) : i.points.map((e2) => new v(e2[0], e2[1])), t3._exporter = (t4, i2) => e.hasM ? new u({ points: t4.map((e2) => [e2.x, e2.y, e2.z, e2.m]), hasZ: true, hasM: true, spatialReference: i2 }) : new u(t4.map((e2) => [e2.x, e2.y, e2.z]), i2);
        break;
      }
      case "polyline": {
        const i = e, n2 = [], o2 = [], { hasZ: l, hasM: s2 } = e;
        let a2 = 0;
        ...
        t3.coordinates = n2, t3._exporter = (t4, i2) => {
          const n3 = e.hasM ? t4.map((e2) => [e2.x, e2.y, e2.z ?? 0, e2.m ?? 0]) : t4.map((e2) => [e2.x, e2.y, e2.z ?? 0]), l2 = o2.map((e2) => n3.slice(e2[0], e2[1]));
          return new P({ paths: l2, hasM: e.hasM, hasZ: true, spatialReference: i2 });
        };
        break;
      }
    }
...
var x = class extends g {
  ...
  selectTilesAtLOD(e) {
    if (e < 0) this.geometry.coordinates.forEach((e2) => e2.tile = null);
      ...
      const { tileInfo: t3, tilemapCache: i } = this.layer, n2 = D(t3, i)[e].level;
  ...
  populateElevationTiles(e) {
    var _a;
    for (const t3 of this.geometry.coordinates) !t3.elevationTile && ((_a = t3.tile) == null ? void 0 : _a.id) && (t3.elevationTile = e[t3.tile.id]);
  }
  remapTiles(e) {
    var _a;
    for (const t3 of this.geometry.coordinates) {
      const i = (_a = t3.tile) == null ? void 0 : _a.id;
      t3.tile = i ? e[i] : null;
    }
  }
  getTilesToFetch() {
...
var _2 = class extends g {
  ...
  _maximumLodForRequests(e) {
    const { tileInfo: t3, tilemapCache: i } = this.layer, n2 = D(t3, i);
  ...
  _selectCandidateTilesCoveringExtentAt(e) {
    ...
    if (null == t3) return;
    const { tileInfo: i, tilemapCache: n2 } = this.layer, o2 = D(i, n2)[e], l = i.tileAt(o2.level, t3.xmin, t3.ymin), s2 = l.extent;
    if (null == s2) return;
  ...
  _tileIsMasked(e) {
    return !!this.maskExtents && this.maskExtents.some((t3) => e.extent && d(t3, e.extent));
  }
};
function E(e, t3, i = 0) {
  const n2 = D(e, t3);
  let o2 = n2.length - 1;
  if (i > 0) {
    const t4 = i / Q(e.spatialReference), s2 = n2.findIndex((e2) => e2.resolution < t4);
    0 === s2 ? o2 = 0 : s2 > 0 && (o2 = s2 - 1);
  }
  return o2;
}
var R = { maximumAutoTileRequests: 20, noDataValue: 0, returnSampleInfo: false, demResolution: "auto", minDemResolution: 0 };
async function q(e, i, n2) {
  let o2;
  const l = e.layer.tileInfo.spatialReference;
  if (i instanceof w ? o2 = await i.project(l, n2) : (await Q2([{ source: i.spatialReference, dest: l }], { signal: n2 }), o2 = O(i, l)), !o2) throw new s("elevation-query:spatial-reference-mismatch", `Cannot query elevation in '${i.spatialReference.wkid}' on an elevation service in '${l.wkid}'`);
...
function A(e, t3) {
  const { tileInfo: i, tilemapCache: n2 } = e.layer, o2 = t3 / Q(i.spatialReference), s2 = D(i, n2);
  let a2 = s2[0], r = 0;
...
function M(e) {
  const { tileInfo: t3, tilemapCache: i } = e.layer, n2 = E(t3, i, e.options.minDemResolution);
  e.selectTilesAtLOD(n2, e.options.maximumAutoTileRequests);
}
function D(e, t3) {
  const i = e.lods;
  if (L(t3)) {
    const { effectiveMinLOD: e2, effectiveMaxLOD: n2 } = t3;
    return i.filter((t4) => t4.level >= e2 && t4.level <= n2);
  }
  return i;
}
async function F(e, t3) {
  const i = e.getTilesToFetch(), o2 = {}, l = e.options.cache, s2 = e.options.noDataValue, a2 = i.map(async (i2) => {
    if (null == i2.id) return;
    const n2 = `${e.layer.uid}:${i2.id}:${s2}`, a3 = null != l ? l.get(n2) : null, r = null != a3 ? a3 : await e.layer.fetchTile(i2.level, i2.row, i2.col, { noDataValue: s2, signal: t3 });
    null != l && l.put(n2, r), o2[i2.id] = new t2(i2, r);
  });
  await h(Promise.allSettled(a2), t3), e.populateElevationTiles(o2);
}
function S(e) {
...
function z(e, t3) {
  ...
  return o2.filter((e2, i2) => {
    for (let n3 = 0; n3 < i2; n3++) {
      ...
      if (i3 && e2.extent && d(i3, e2.extent)) return t3 && t3(e2), false;
    }
    return true;
  });
}
async function b(e, t3) {
  const n2 = await e.geometry.project(e.outSpatialReference, t3);
  n(n2);
...
function k(e) {
  const t3 = e.layer.tileInfo, i = Q(t3.spatialReference);
...
//# sourceMappingURL=chunk-4GBV7CTQ.js.map
