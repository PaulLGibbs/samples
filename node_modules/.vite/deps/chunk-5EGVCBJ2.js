...
function c(t2) {
  ...
  if (d3 = s2(t2, d3, h3), d3 = s2(t2, d3, w3), d3 = s2(t2, d3, g2), d3 = s2(t2, d3, p2), d3 !== t2.byteLength) throw new s("lepcc-decode-error", "Bad length");
  let m2 = 0, U = 0;
  for (let e2 = 0; e2 < h3.length; e2++) {
    ...
    for (let o2 = 0; o2 < w3[e2]; o2++) {
      t3 += g2[m2];
      const e3 = p2[m2];
      f3[3 * m2] = Math.min(u3.maxX, u3.minX + 2 * u3.errorX * t3), f3[3 * m2 + 1] = Math.min(u3.maxY, u3.minY + 2 * u3.errorY * U), f3[3 * m2 + 2] = Math.min(u3.maxZ, u3.minZ + 2 * u3.errorZ * e3), m2++;
    }
  }
...
function d(o2, r2, n3) {
  ...
  for (let e2 = 0; e2 < a4; e2++) {
    for (; w3 < s4; ) h3 |= f3[g2] << w3, w3 += 8, g2 += 1;
    n3[e2] = h3 & p2, h3 >>>= s4, w3 -= s4, w3 + s4 > 32 && (h3 |= f3[g2 - 1] >> 8 - w3);
  }
  return r2 + g2;
}
...
function p(e2, t2) {
  return !(!e2 || !e2.compressedAttributes || "draco" !== e2.compressedAttributes.encoding) ? v(e2.compressedAttributes.attributes) : e2 ? g(e2) : m(t2);
}
function m(e2) {
  const t2 = { isDraco: false, isLegacy: true, color: false, normal: false, uv0: false, uvRegion: false, featureIndex: false };
  for (const n3 of e2.ordering) if (e2.vertexAttributes[n3]) switch (n3) {
    case "position":
      break;
    case "normal":
      t2.normal = true;
...
//# sourceMappingURL=chunk-5EGVCBJ2.js.map
