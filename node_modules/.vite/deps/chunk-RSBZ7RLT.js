...
var _Link = class _Link extends LitElement {
  ...
  render() {
    ...
    return InteractiveContainer({ disabled: this.disabled, children: html2`<${DynamicHtmlTag} class=${safeClassMap({ [CSS_UTILITY.rtl]: dir === "rtl" })} download=${(childElType === "a" ? download === true || download === "" ? "" : download || null : null) ?? nothing} href=${(childElType === "a" && this.href) ?? nothing} @click=${this.childElClickHandler} rel=${(childElType === "a" && this.rel) ?? nothing} .role=${role} tabindex=${tabIndex ?? nothing} target=${(childElType === "a" && this.target) ?? nothing} ${ref(this.storeTagRef)}>${html`${this.iconStart ? iconStartEl : null}<slot></slot>${this.iconEnd ? iconEndEl : null}`}</${DynamicHtmlTag}>` });
  }
};
...
@esri/calcite-components/dist/components/calcite-link/customElement.js:
  ...
  v3.2.1 *)
*/
//# sourceMappingURL=chunk-RSBZ7RLT.js.map
