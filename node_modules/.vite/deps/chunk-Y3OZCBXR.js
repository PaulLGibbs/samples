...
var b = class _b {
  ...
  remove(t, e2 = null) {
    ...
    const n2 = g2.acquire();
    for (const o of t) {
      const t2 = e2 ?? w(this.objectToBoundingSphere(o), J);
  ...
  findClosest(t, e2, n2, o = () => true, s3 = 1 / 0) {
    let h = 1 / 0, a = 1 / 0, d = null;
    const u2 = F(t, e2), c = (i2) => {
      if (--s3, !o(i2)) return;
      const r3 = this.objectToBoundingSphere(i2);
      if (!s2(n2, r3)) return;
      const u3 = v2(t, e2, U(r3)), c2 = u3 - r3[3], f = u3 + r3[3];
      c2 < h && (h = c2, a = f, d = i2);
    };
    return j(this._root, (o2) => {
      if (s3 <= 0 || !s2(n2, o2.bounds)) return false;
      g(I2, u2, o2.halfSize), u(I2, I2, U(o2.bounds));
      if (v2(t, e2, I2) > a) return false;
      const h2 = o2.node;
      return h2.terminals.forAll((t2) => c(t2)), null !== h2.residents && h2.residents.forAll((t2) => c(t2)), true;
    }, t, e2), d;
  }
  forEachInDepthRange(t, e2, n2, o, s3, h, a) {
    let d = -1 / 0, u2 = 1 / 0;
    const c = { setRange: (t2) => {
      n2 === _b.DepthOrder.FRONT_TO_BACK ? (d = Math.max(d, t2.near), u2 = Math.min(u2, t2.far)) : (d = Math.max(d, -t2.far), u2 = Math.min(u2, -t2.near));
    } };
    c.setRange(o);
    const f = v2(e2, n2, t), _ = F(e2, n2), p = F(e2, -n2), g3 = (t2) => {
      if (!a(t2)) return;
      const o2 = this.objectToBoundingSphere(t2), i2 = U(o2), r3 = v2(e2, n2, i2) - f, _2 = r3 - o2[3], p2 = r3 + o2[3];
      _2 > u2 || p2 < d || !s2(h, o2) || s3(t2, c);
    };
    j(this._root, (t2) => {
      if (!s2(h, t2.bounds)) return false;
      g(I2, _, t2.halfSize), u(I2, I2, U(t2.bounds));
      if (v2(e2, n2, I2) - f > u2) return false;
      g(I2, p, t2.halfSize), u(I2, I2, U(t2.bounds));
      if (v2(e2, n2, I2) - f < d) return false;
      const o2 = t2.node;
  ...
  forEachNeighbor(t, e2) {
    ...
    S(this._root, (t2) => {
      ...
      if (v(U(t2.bounds), o) - i3 * i3 > 0) return false;
      const a = t2.node;
  ...
  _grow(t, e2) {
    if (0 !== e2 && (D(t, e2, (t2) => this.objectToBoundingSphere(t2), W), C(W[3]) && !this._fitsInsideTree(W))) if (N2(this._root.node)) w(W, this._root.bounds), this._root.halfSize = 1.25 * this._root.bounds[3], this._root.updateBoundsRadiusFromHalfSize();
    else {
  ...
  _rebuildTree(t, e2) {
    s(U(V), U(e2.bounds)), V[3] = e2.halfSize, D([t, V], 2, (t2) => t2, G);
  ...
  _rootBoundsForRootAsSubNode(t) {
    ...
    for (let h = 0; h < 3; h++) {
      const t2 = i2[h] - r3 - (n2[h] - e2), s4 = n2[h] + e2 - (i2[h] + r3), a = Math.max(0, Math.ceil(t2 / (2 * r3))), l = Math.max(0, Math.ceil(s4 / (2 * r3))) + 1, d = 2 ** Math.ceil(Math.log(a + l) * Math.LOG2E);
      o = Math.max(o, d), U2[h].min = a, U2[h].max = l;
    }
    for (let h = 0; h < 3; h++) {
      let t2 = U2[h].min, e3 = U2[h].max;
      const n3 = (o - (t2 + e3)) / 2;
      t2 += Math.ceil(n3), e3 += Math.floor(n3);
      const s4 = i2[h] - r3 - t2 * r3 * 2;
      k2[h] = s4 + (e3 + t2) * r3;
    }
    const s3 = o * r3;
    return k2[3] = s3 * w2, g2.acquire().initFrom(null, k2, s3, 0);
  }
...
var g2 = class _g {
  ...
  initFrom(t, e2, n2, o = this.depth) {
    return this.node = null != t ? t : _g.createEmptyNode(), e2 && w(e2, this.bounds), this.halfSize = n2, this.depth = o, this;
  }
  increaseHalfSize(t) {
...
function x(t, n2) {
  return n2 >= 0 && (t.children[n2] = null), !!N2(t) && (null === t.residents && (t.residents = new r({ shrink: true })), true);
}
function T(t, e2) {
  return B(U(e2.bounds), 2 * -e2.halfSize, L), B(U(e2.bounds), 2 * e2.halfSize, H2), i(t.origin, t.direction, L, H2);
}
function z(t, e2, n2) {
  return B(U(e2.bounds), 2 * -e2.halfSize, L), B(U(e2.bounds), 2 * e2.halfSize, H2), n2.applyToMinMax(L, H2), i(t.origin, t.direction, L, H2);
}
function N2(t) {
...
function D(t, e2, n2, i2) {
  if (1 === e2) {
    ...
    for (let o = 0; o < e2; o++) {
      const e3 = n2(t[o]);
      C(e3[3]) && (M(L, e3), A(H2, e3));
    }
    I(U(i2), L, H2, 0.5), i2[3] = Math.max(H2[0] - L[0], H2[1] - L[1], H2[2] - L[2]) / 2;
  }
}
function R(t, e2, n2) {
  if (!Q.length) for (let o = 0; o < 8; ++o) Q.push({ index: 0, distance: 0 });
  for (let o = 0; o < 8; ++o) {
    const n3 = E2[o];
    Q.data[o].index = o, Q.data[o].distance = v2(t, e2, n3);
  }
...
var P = [r2(-1, -1, -1), r2(-1, -1, 1), r2(-1, 1, -1), r2(-1, 1, 1), r2(1, -1, -1), r2(1, -1, 1), r2(1, 1, -1), r2(1, 1, 1)];
var w2 = Math.sqrt(3);
var q = [null];
function y(t) {
  return q[0] = t, q;
...
//# sourceMappingURL=chunk-Y3OZCBXR.js.map
