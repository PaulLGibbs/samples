...
function e(t2, n2) {
  return 0 === n2 ? 0 : Math.round(t2 / n2) * n2;
}
function u(t2) {
  return !(t2 & t2 - 1);
}
...
function a(t2) {
  return 10 ** Math.ceil(Math.LOG10E * Math.log(t2));
}
function o(t2, n2, r2) {
  return t2 + (n2 - t2) * r2;
}
function c(t2, n2, r2, e2, u2) {
  return o(e2, u2, (t2 - n2) / (r2 - n2));
}
function s(t2) {
...
function P(t2, n2, r2 = w) {
  ...
  const e2 = p(t2), u2 = p(n2);
  if (e2 < 0 != u2 < 0) return false;
  return !((e2 < u2 ? u2 - e2 : e2 - u2) > B);
}
function A(t2, n2, r2 = 1e-6) {
  if (t2 === n2) return true;
  if (!Number.isFinite(t2) || !Number.isFinite(n2)) return false;
  const e2 = Math.abs(t2 - n2), u2 = Math.abs(t2), i2 = Math.abs(n2);
  if (0 === t2 || 0 === n2 || u2 < 1e-12 && i2 < 1e-12) {
    if (e2 > 0.01 * r2) return false;
  } else if (e2 / (u2 + i2) > r2) return false;
  return true;
}
function S(t2) {
...
function L(t2, n2, e2) {
  const u2 = r((e2 - t2) / (n2 - t2), 0, 1);
  return u2 * u2 * (3 - 2 * u2);
}
function T(t2) {
  const n2 = t2[0] * t2[0] + t2[1] * t2[1] + t2[2] * t2[2], r2 = t2[3] * t2[3] + t2[4] * t2[4] + t2[5] * t2[5], e2 = t2[6] * t2[6] + t2[7] * t2[7] + t2[8] * t2[8];
  return !(m(n2, 1) && m(r2, 1) && m(e2, 1));
}
function _(t2, n2) {
  return (t2 % n2 + n2) % n2;
}
...
function v(t2, n2, r2) {
  ...
  let e2 = n2.toString().split("e");
  return e2 = (n2 = Math[t2](+(e2[0] + "e" + (e2[1] ? +e2[1] - r2 : -r2)))).toString().split("e"), +(e2[0] + "e" + (e2[1] ? +e2[1] + r2 : r2));
}
...
//# sourceMappingURL=chunk-WTKIQJGU.js.map
