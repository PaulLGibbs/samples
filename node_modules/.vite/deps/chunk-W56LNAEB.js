...
function y(e6, n, i2) {
  ...
  switch (t4.type) {
    case "range": {
      const { min: i3, max: l3 } = f3(e6, t4);
      if (null != i3 && +n < i3 || null != l3 && +n > l3) return r.VALUE_OUT_OF_RANGE;
      break;
    }
    case "coded-value":
    case "codedValue":
      if (null == t4.codedValues || t4.codedValues.every((e7) => null == e7 || e7.code !== n)) return r.INVALID_CODED_VALUE;
  }
  return null;
}
function f3(e6, n) {
  ...
  const t4 = "range" in i2 ? i2.range[0] : i2.minValue, r2 = "range" in i2 ? i2.range[1] : i2.maxValue, l3 = u4(e6);
...
function g(e6, n) {
  switch (n.type) {
    case "range": {
      const { min: i2, max: t4 } = f3(e6, n);
      return a(e6) || s3(e6) || o2(e6) ? !!i2 && !!t4 : "number" == typeof i2 && Number.isFinite(i2) && "number" == typeof t4 && Number.isFinite(t4);
    }
    case "codedValue":
    case "coded-value": {
      const i2 = typeof n.codedValues[0].code;
      if ("string" === i2 && p(e6) || "number" === i2 && d(e6)) return true;
    }
  }
  return false;
}
...
function e5(t4) {
  var _a, _b;
  return ((_b = (_a = t4.match(u5)) == null ? void 0 : _a[1]) == null ? void 0 : _b.replace(/\\'/g, "'")) ?? null;
}
...
function p2(e6) {
  ...
  return e6.trim().replaceAll(d2, "_").replace(c3, "F$1") || null;
}
...
var y2 = ["field", "normalizationField"];
function g2(e6, n) {
  if (null != e6 && null != n) {
    for (const i2 of Array.isArray(e6) ? e6 : [e6]) if (F(m3, i2, n), "visualVariables" in i2 && i2.visualVariables) for (const e7 of i2.visualVariables) F(y2, e7, n);
  }
}
function F(e6, t4, r2) {
...
async function $(e6) {
  ...
  return await L(n, e6), Array.from(n).sort();
}
...
async function k(e6) {
  if (!e6) return [];
  const n = "timeInfo" in e6 && e6.timeInfo;
  return n ? w(e6.fieldsIndex, [e6.trackIdField, n.startField, n.endField]) : [];
}
function C(e6) {
...
async function K(e6) {
  if (!e6) return [];
  const n = /* @__PURE__ */ new Set();
  return await Q(n, e6), Array.from(n).sort();
}
...
function $e(e6, n) {
  const i2 = f3(e6, n);
  return i2 || (ge(e6) ? Le(e6.type) : void 0);
}
function Le(e6) {
  switch (e6) {
    case "esriFieldTypeSmallInteger":
...
function Oe(e6) {
  if (!ie(e6)) return null;
  if (Number.isInteger(e6)) {
    if (e6 >= De.min && e6 <= De.max) return "esriFieldTypeSmallInteger";
    if (e6 >= Me.min && e6 <= Me.max) return "esriFieldTypeInteger";
    if (e6 >= Ve.min && e6 <= Ve.max) return "esriFieldTypeBigInteger";
  }
  return e6 >= Re.min && e6 <= Re.max ? "esriFieldTypeSingle" : "esriFieldTypeDouble";
}
!function(e6) {
...
async function Ge(e6, n) {
  const i2 = /* @__PURE__ */ new Set();
  for (const t4 of n) await _(i2, e6.fieldsIndex, t4);
  return Array.from(i2).sort();
}
function Pe(e6) {
  return !!e6 && ["raster.itempixelvalue", "raster.servicepixelvalue"].some((n) => e6.toLowerCase().startsWith(n));
}
async function qe(e6, n) {
  const i2 = /* @__PURE__ */ new Set();
  return (e6 == null ? void 0 : e6.collectRequiredFields) && await e6.collectRequiredFields(i2, n), Array.from(i2).sort();
}
function Xe(e6) {
...
//# sourceMappingURL=chunk-W56LNAEB.js.map
