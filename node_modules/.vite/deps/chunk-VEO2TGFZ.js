...
function i2(e4, t5, n3, o3, i5, l3) {
  const { iconRotationAlignment: a, textRotationAlignment: c2, iconTranslate: h, iconTranslateAnchor: u, textTranslate: d, textTranslateAnchor: y } = o3;
  let x = 0;
...
var l2 = class {
  ...
  work(e4) {
    ...
    for (; this._currentLayerCursor < this._symbols.length; this._currentLayerCursor++, this._currentSymbolCursor = 0) {
      ...
      for (; this._currentSymbolCursor < t6.symbols.length; this._currentSymbolCursor++) {
        ...
        const l3 = s.unique;
        if (!l3.show) continue;
        const { iconAllowOverlap: a, iconIgnorePlacement: c2, textAllowOverlap: h, textIgnorePlacement: u } = r5;
  ...
  _getProperties(e4) {
    ...
    const c2 = s.getPaintValue("icon-translate", n3), h = s.getPaintValue("icon-translate-anchor", n3), u = s.getPaintValue("text-translate", n3), d = s.getPaintValue("text-translate-anchor", n3), y = { iconAllowOverlap: s.getLayoutValue("icon-allow-overlap", n3), iconIgnorePlacement: s.getLayoutValue("icon-ignore-placement", n3), textAllowOverlap: s.getLayoutValue("text-allow-overlap", n3), textIgnorePlacement: s.getLayoutValue("text-ignore-placement", n3), iconRotationAlignment: l3, textRotationAlignment: a, iconTranslateAnchor: h, iconTranslate: c2, textTranslateAnchor: d, textTranslate: u };
...
function e3(o3, e4) {
  if (o3.priority - e4.priority) return o3.priority - e4.priority;
  const t5 = o3.tile.key, i5 = e4.tile.key;
  return t5.world - i5.world ? t5.world - i5.world : t5.level - i5.level ? t5.level - i5.level : t5.row - i5.row ? t5.row - i5.row : t5.col - i5.col ? t5.col - i5.col : o3.xTile - e4.xTile ? o3.xTile - e4.xTile : o3.yTile - e4.yTile;
}
var t3 = class {
  get running() {
    return this._running;
  }
  constructor(o3, e4, t5, i5, s, n3) {
    this._visibleTiles = o3, this._symbolRepository = e4, this._createCollisionJob = t5, this._assignTileSymbolsOpacity = i5, this._symbolLayerSorter = s, this._isLayerVisible = n3, this._selectionJob = null, this._selectionJobCompleted = false, this._collisionJob = null, this._collisionJobCompleted = false, this._opacityJob = null, this._opacityJobCompleted = false, this._running = true;
  }
  setScreenSize(o3, e4) {
    this._screenWidth === o3 && this._screenHeight === e4 || this.restart(), this._screenWidth = o3, this._screenHeight = e4;
  }
  restart() {
  ...
  continue(o3) {
    if (this._selectionJob || (this._selectionJob = this._createSelectionJob()), !this._selectionJobCompleted) {
      ...
      if (this._selectionJobCompleted = true, 0 === (o3 = Math.max(0, o3 - (performance.now() - e4)))) return false;
    }
    if (this._collisionJob || (this._collisionJob = this._createCollisionJob(this._selectionJob.sortedSymbols, this._screenWidth, this._screenHeight)), !this._collisionJobCompleted) {
      ...
      if (this._collisionJobCompleted = true, 0 === (o3 = Math.max(0, o3 - (performance.now() - e4)))) return false;
    }
    if (this._opacityJob || (this._opacityJob = this._createOpacityJob()), !this._opacityJobCompleted) {
      ...
      if (this._opacityJobCompleted = true, 0 === (o3 = Math.max(0, o3 - (performance.now() - e4)))) return false;
    }
  ...
  _createSelectionJob() {
    ...
    for (let e4 = 0; e4 < o3.length; e4++) {
      const t6 = o3[e4];
      for (let o4 = 0; o4 < t6.uniqueSymbols.length; o4++) {
        const e5 = t6.uniqueSymbols[o4];
        for (const o5 of e5.tileSymbols) o5.selectedForRendering = false;
      }
    }
    const t5 = [];
    let i5 = 0, s = 0;
    ...
    function r5(r6) {
      ...
      for (; s < o3.length; s++, i5 = 0) {
        const e4 = o3[s], h = e4.styleLayerUID;
        if (!n3(h)) {
          t5[s] || (t5[s] = { styleLayerUID: h, symbols: [] });
          continue;
        }
        t5[s] = t5[s] || { styleLayerUID: h, symbols: [] };
        const a = t5[s];
        for (; i5 < e4.uniqueSymbols.length; i5++) {
          if (l4 = e4.uniqueSymbols[i5], i5 % 100 == 99 && performance.now() - c2 > r6) return false;
          let o4 = null, t6 = false, s2 = false;
          for (const e5 of l4.tileSymbols) if (!s2 || !t6) {
            const i6 = e5.tile;
            (!o4 || i6.isCoverage || i6.neededForCoverage && !t6) && (o4 = e5, (i6.neededForCoverage || i6.isCoverage) && (s2 = true), i6.isCoverage && (t6 = true));
          }
      ...
      for (const o4 of t5) o4.symbols.sort(e3);
      return true;
    }
  ...
  _createOpacityJob() {
    const o3 = this._assignTileSymbolsOpacity, e4 = this._visibleTiles;
    let t5 = 0;
    function s(e5, t6) {
      for (const o4 of e5.symbols.values()) i3(o4, t6);
      o3(e5, t6);
      for (const o4 of e5.childrenTiles) s(o4, t6);
    }
    return { work(o4) {
...
function i3(e4, t5) {
  for (const i5 of e4) {
    const e5 = i5.unique;
    for (const i6 of e5.parts) {
      const s = i6.targetOpacity > 0.5 ? 1 : -1;
      i6.startOpacity += s * ((t5 - i6.startTime) / e), i6.startOpacity = Math.min(Math.max(i6.startOpacity, 0), 1), i6.startTime = t5, i6.targetOpacity = e5.show && i6.show ? 1 : 0;
    }
  }
}
...
var r4 = class {
  ...
  add(s, l3) {
    ...
    n3 || (n3 = { symbols: /* @__PURE__ */ new Map() }, this._tiles.set(s.id, n3));
    ...
    for (const [f, u] of y) {
      ...
      if (s2 >= o2) {
        ...
        a.set(f, { flat: u, index: o3 }), n3.symbols.set(f, { flat: u, index: o3 });
        for (const e4 of u) o3.getCell(e4.xTile, e4.yTile).push(e4);
      } else a.set(f, { flat: u }), n3.symbols.set(f, { flat: u });
    }
  ...
  querySymbols(e4, l3, o3, t5) {
    ...
    for (const r5 of n3) {
      ...
      for (const r6 of n4) {
        ...
        n5 && c(n5, e4, l3 * (window.devicePixelRatio || 1), o3) && i5.push({ vtlSymbol: n5, styleLayerUID: t6, tileKey: n5.tile.key });
      }
    }
    return i5;
  }
  _removeSymbols(e4) {
    for (const [s, { flat: l3 }] of e4) for (const e5 of l3) {
  ...
  _matchSymbols(e4, s, o3) {
    ...
    for (const [i5, r5] of o3) {
      ...
      for (const t6 of r5) {
        ...
        i6 >= 0 && i6 < this.tileCoordRange && n3 >= 0 && n3 < this.tileCoordRange && o4.push({ symbol: t6, xTransformed: i6, yTransformed: n3 });
      }
  ...
  _createUniqueSymbolLayerArray() {
    ...
    for (const [t5, i5] of e4) {
      ...
      s[o3] = { styleLayerUID: t5, uniqueSymbols: e5 }, o3++;
    }
    return s;
  }
};
...
//# sourceMappingURL=chunk-VEO2TGFZ.js.map
