import {
  Fe
} from "./chunk-TUHMNOE3.js";
import {
  s
} from "./chunk-6F2XPKOO.js";
import {
  p
} from "./chunk-PDGM6FOO.js";
import {
  b as b2
...
var b3 = class extends a2.ClonableMixin(n.IdentifiableMixin(g)) {
  ...
  get state() {
    const { _queryAbortController: t, _queryFeatureCountAbortController: e, _queryPageAbortController: o2, canQuery: s2, _loaded: r2, canLoad: i, source: a3 } = this;
    return e || i && !r2 ? "loading" : t || o2 ? "querying" : !s2 || "popup" === a3 && 0 === this.featureCount ? "disabled" : "ready";
  }
  get utilityNetwork() {
  ...
  async _findLayersBySourceId(t) {
    const { utilityNetwork: e, map: o2 } = this, s2 = (t2) => {
      const o3 = t2;
    ...
    const r2 = e.getLayerIdBySourceId(t), i = o2.allLayers.filter(s2), a3 = o2.allTables.filter(s2), n2 = i.concat(a3).toArray();
  ...
  async _queryLayer(t, e, o2, s2, r2) {
    const i = t.fieldsIndex.get(o.assetGroup), a3 = t.fieldsIndex.get(o.assetType), n2 = null != o2, c = null != s2, l2 = "(" + e.map((t2) => `'${t2}'`).join(", ") + ")", u = n2 ? ` AND ${i == null ? void 0 : i.name} = ${o2}` : "", d3 = n2 && c ? ` AND ${a3 == null ? void 0 : a3.name} = ${s2}` : "", y = `${t.globalIdField} IN ${l2}` + u + d3, h = new b2({ outFields: ["*"], cacheHint: this.supportsCacheHint, where: y });
    return await this._queryAll(h, t, { signal: r2 == null ? void 0 : r2.signal });
  }
  async _createAssociationFeatureObjects(t, e, o2, s2, r2, i) {
    if (0 === t.length) return [];
    const a3 = /* @__PURE__ */ new Map();
    for (const [c, l2] of e) {
      const t2 = await this._findLayersBySourceId(c);
      for (const e2 of t2) {
        (await this._queryLayer(e2, l2, s2, r2, i)).forEach((t3) => {
    ...
    return await Promise.all(t.toArray().map(async (t2) => {
      const { fromNetworkElement: e2, toNetworkElement: s3 } = t2, r3 = e2.globalId === o2 ? s3 : e2, i2 = a3.get(r3.globalId) ?? [];
      await Promise.all(i2.map(async (e3) => {
        ...
        if (e3.layer && "getFeatureTitle" in e3.layer) {
          const s4 = await e3.layer.getFeatureTitle(e3);
          n2.push({ title: s4, association: t2, feature: e3, terminalName: o3 });
  ...
  _parseFeatureObjects(t, e) {
    t.forEach((t2) => {
      const o2 = t2 == null ? void 0 : t2.feature, s2 = o2.layer, i = e.get(s2) ?? new V();
      i.add(t2), e.set(s2, i);
    });
  }
  async _queryAll(t, e, o2) {
    ...
    do {
      ...
      s2.push(...a3.features), i = a3.exceededTransferLimit, i && (r2 += a3.features.length);
    } while (i);
    return s2;
  }
  async _queryAssociations(t) {
    const { layer: e, globalId: o2, associationTypes: s2, utilityNetwork: r2, canQuery: i } = this;
    if (await Promise.allSettled([e == null ? void 0 : e.load(), r2 == null ? void 0 : r2.load()]), this._clearAssociations(), !(i && e && s2 && r2 && o2)) return;
    ...
    s2.forEach((t2) => {
      ...
      const { toNetworkElement: e2, fromNetworkElement: s3 } = t2;
      switch (t2.associationType) {
        ...
        case "junction-edge-to-connectivity":
          if ((s3 == null ? void 0 : s3.globalId) === o2) {
            if (this._shouldDiscardNetworkElement(e2, "connectivity", d3)) break;
            (_a = u.get("connectivity")) == null ? void 0 : _a.push(e2.globalId);
          } else {
            if (this._shouldDiscardNetworkElement(s3, "connectivity", d3)) break;
            (_b = u.get("connectivity")) == null ? void 0 : _b.push(s3.globalId);
          }
        ...
        case "containment":
          if ((s3 == null ? void 0 : s3.globalId) === o2) {
            ...
            if (this._shouldDiscardNetworkElement(s3, "container", d3)) break;
            (_d = u.get("container")) == null ? void 0 : _d.push(s3.globalId), this.containerAssociations.add(t2);
          }
          break;
        case "attachment":
          if ((s3 == null ? void 0 : s3.globalId) === o2) {
            ...
            if (this._shouldDiscardNetworkElement(s3, "structure", d3)) break;
            (_f = u.get("structure")) == null ? void 0 : _f.push(s3.globalId), this.structureAssociations.add(t2);
          }
      }
    });
    const y = s2.map(async (e2) => {
      const { associatedNetworkSourceId: o3, associatedAssetGroup: s3, associatedAssetType: r3 } = e2, i2 = u.get(e2.type), a4 = null != s3 ? await this._countAssociatedFeatures(o3, i2, s3, r3, t) : i2.length;
      switch (e2.type) {
        case "attachment":
  ...
  async _countAssociatedFeatureCount(t, e, o2, s2, r2) {
    const i = t.fieldsIndex.get(o.assetGroup), a3 = t.fieldsIndex.get(o.assetType), n2 = null != s2, c = "(" + e.map((t2) => `'${t2}'`).join(", ") + ")", l2 = ` AND ${i == null ? void 0 : i.name} = ${o2}`, u = n2 ? ` AND ${a3 == null ? void 0 : a3.name} = ${s2}` : "", d3 = `${t.globalIdField} IN ${c}` + l2 + u;
    return t.queryFeatureCount({ where: d3, outFields: ["*"], returnGeometry: false }, { signal: r2 == null ? void 0 : r2.signal });
  }
  async _countAssociatedFeatures(t, e, o2, s2, r2) {
    if (0 === e.length) return 0;
    const i = (await this._findLayersBySourceId(t)).map(async (t2) => this._countAssociatedFeatureCount(t2, e, o2, s2, r2));
    return (await Promise.all(i)).reduce((t2, e2) => t2 + e2, 0);
  }
  async _queryAssociatedFeatures(t, e) {
    const { layer: o2, globalId: s2, associationTypes: r2, utilityNetwork: i, canQuery: a3, associationFeatures: n2 } = this;
    if (await Promise.allSettled([o2 == null ? void 0 : o2.load(), i == null ? void 0 : i.load()]), !(a3 && o2 && r2 && i)) return;
    const c = this._getAssociationsByType(t.type), { associatedAssetGroup: l2, associatedAssetType: u } = t, d3 = /* @__PURE__ */ new Map();
    c.forEach((t2) => {
      const { fromNetworkElement: e2, toNetworkElement: o3 } = t2, { networkSourceId: r3, elementGlobalId: i2 } = e2.globalId === s2 ? { networkSourceId: o3.networkSourceId, elementGlobalId: o3.globalId } : { networkSourceId: e2.networkSourceId, elementGlobalId: e2.globalId }, a4 = d3.get(r3) || [];
      a4.push(i2), d3.set(r3, a4);
    });
    const y = await this._createAssociationFeatureObjects(c, d3, s2, l2, u, e);
  ...
  _shouldDiscardNetworkElement(t, e, o2) {
    var _a;
    if (!t) return false;
    const { networkSourceIdsInUse: s2 } = this, { networkSourceId: r2 } = t, i = (_a = o2.get(e)) == null ? void 0 : _a.associatedNetworkSourceId, a3 = s2.has(r2);
    return null != i && i !== r2 || !a3;
  }
};
...
//# sourceMappingURL=chunk-TPC3USN3.js.map
