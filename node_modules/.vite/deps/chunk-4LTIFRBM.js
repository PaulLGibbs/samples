...
var j2 = w = class extends S {
  static create(e = {}) {
    const { resolutionFactor: t2 = 1, scales: o3, size: r3 = 256, spatialReference: i = g.WebMercator, numLODs: l = 24 } = e;
    if (!N(i)) {
      const e2 = [];
      if (o3) for (let t3 = 0; t3 < o3.length; t3++) {
        ...
        let t3 = 5e-4;
        for (let o4 = l - 1; o4 >= 0; o4--) e2.unshift(new p({ level: o4, scale: t3, resolution: t3 })), t3 *= 2;
      }
      return new w({ dpi: 96, lods: e2, origin: new j(0, 0, i), size: [r3, r3], spatialReference: i });
    }
    const n = d(i), p2 = e.origin ? new j({ x: e.origin.x, y: e.origin.y, spatialReference: i }) : new j(n ? { x: n.origin[0], y: n.origin[1], spatialReference: i } : { x: 0, y: 0, spatialReference: i }), u2 = 96, d2 = 1 / (Q(i) * 39.37 * u2), g2 = [];
    if (o3) for (let s2 = 0; s2 < o3.length; s2++) {
      const e2 = o3[s2], t3 = e2 * d2;
      g2.push(new p({ level: s2, scale: e2, resolution: t3 }));
    }
    else {
      let e2 = P(i) ? 512 / r3 * 5916575275917094e-7 : 256 / r3 * 591657527591555e-6;
      const o4 = Math.ceil(l / t2);
    ...
    return new w({ dpi: u2, lods: g2, origin: p2, size: [r3, r3], spatialReference: i });
  }
  constructor(e) {
  ...
  tileAt(e, t2, o3, s2) {
    ...
    if (!r3) return null;
    let i, l;
    if ("number" == typeof t2) i = t2, l = o3;
    else if (s(t2.spatialReference, this.spatialReference)) i = t2.x, l = t2.y, s2 = o3;
    else {
    ...
    return s2 || (s2 = new t(null, 0, 0, 0, u())), s2.level = e, s2.row = Math.floor((this.origin.y - l) / p2 + 1e-3), s2.col = Math.floor((i - this.origin.x) / n + 1e-3), this.updateTileInfo(s2), s2;
  }
  updateTileInfo(e, t2 = w.ExtrapolateOptions.NONE) {
    ...
    if (!o3) return;
    const s2 = e.level - o3.level, r3 = o3.resolution * this.size[0] / 2 ** s2, i = o3.resolution * this.size[1] / 2 ** s2;
    e.id = `${e.level}/${e.row}/${e.col}`, e.extent || (e.extent = u()), e.extent[0] = this.origin.x + e.col * r3, e.extent[1] = this.origin.y - (e.row + 1) * i, e.extent[2] = e.extent[0] + r3, e.extent[3] = e.extent[1] + i;
  }
  upsampleTile(e) {
  ...
  getTileBounds(e, t2) {
    ...
    const { resolution: s2 } = o3, r3 = s2 * this.size[0], i = s2 * this.size[1];
    return e[0] = this.origin.x + t2.col * r3, e[1] = this.origin.y - (t2.row + 1) * i, e[2] = e[0] + r3, e[3] = e[1] + i, e;
  }
  lodAt(e) {
    var _a;
  ...
  getCompatibleForVTL(e) {
    ...
    for (let r3 = 0; r3 < s2; r3++) {
      const e2 = r3 + t2, { scale: s3, resolution: i } = e2 >= 0 ? this.lods[e2] : { scale: 2 * this.lods[0].scale, resolution: 2 * this.lods[0].resolution };
      o3.push(new p({ level: r3, scale: s3, resolution: i }));
    }
...
//# sourceMappingURL=chunk-4LTIFRBM.js.map
