...
var r2 = class {
  constructor(t, e2, i2, s2 = 0) {
    this.name = t, this._storage = e2, this._defaultPriority = s2, this.id = o++ + ":", this.size = 0, this.maxSize = -1, this._removeFunc = false, this._hit = 0, this._miss = 0, this._storage.register(this), i2 && (this._storage.registerRemoveFunc(this.id, i2), this._removeFunc = true);
  }
  destroy() {
  ...
  put(t, e2, i2 = this._defaultPriority) {
    this._storage.put(this, t, e2, e2.cachedMemory, i2);
  }
  pop(t) {
  ...
  updateSize(t, e2) {
    this._storage.updateSize(this, t, e2, e2.cachedMemory);
  }
  clear() {
...
var h = class {
  ...
  put(t, r3, h2, o2, _) {
    r3 = t.id + r3;
    const a = this._db.get(r3);
    if (a && (this._size -= a.size, t.size -= a.size, this._db.delete(r3), a.entry !== h2 && this._notifyRemove(r3, a.entry, a.size, s.ALL)), o2 > this._maxSize) return void this._notifyRemove(r3, h2, o2, s.ALL);
    if (void 0 === h2) return void console.warn("Refusing to cache undefined entry ");
    if (!o2 || o2 < 0) return console.warn(`Refusing to cache entry with size ${o2} for key ${r3}`), void this._notifyRemove(r3, h2, 0, s.ALL);
    const c = 1 + Math.max(_, i) - e;
    this._db.set(r3, new n(h2, o2, c)), this._size += o2, t.size += o2, this._checkSize();
  }
  updateSize(t, e2, i2, r3) {
    e2 = t.id + e2;
    const h2 = this._db.get(e2);
    if (h2 && h2.entry === i2) {
      for (this._size -= h2.size, t.size -= h2.size; r3 > this._maxSize; ) {
        const t2 = this._notifyRemove(e2, i2, r3, s.SOME);
        if (!(null != t2 && t2 > 0)) return void this._db.delete(e2);
        r3 = t2;
      }
      h2.size = r3, this._size += r3, t.size += r3, this._checkSize();
    }
  }
  pop(t, e2) {
    e2 = t.id + e2;
  ...
  get performanceInfo() {
    ...
    this._db.forEach((t2, e2) => {
      ...
      s2[r4] = (s2[r4] || 0) + t2.size, this._users.forAll((s3) => {
        const { id: r5, name: h3 } = s3;
        if (e2.startsWith(r5)) {
          const e3 = i2[h3] || 0;
          i2[h3] = e3 + t2.size;
        }
    ...
    this._users.forAll((t2) => {
      ...
      if ("hitRate" in t2 && "number" == typeof t2.hitRate && !isNaN(t2.hitRate) && t2.hitRate > 0) {
        const s3 = i2[e2] || 0;
        i2[e2] = s3, r3[e2] = Math.round(100 * t2.hitRate) + "%";
      } else r3[e2] = "0%";
    });
    const h2 = Object.keys(i2);
    h2.sort((t2, e2) => i2[e2] - i2[t2]), h2.forEach((e2) => t[e2] = Math.round(i2[e2] / 2 ** 20) + "MB / " + r3[e2]);
    for (let o2 = s2.length - 1; o2 >= 0; --o2) {
      const i3 = s2[o2];
      i3 && (t["Priority " + (o2 + e - 1)] = Math.round(i3 / this._size * 100) + "%");
    }
    return t;
  }
  resetStats() {
  ...
  _notifyRemove(t, e2, i2, s2) {
    ...
    return this._removeFuncs.some((h2) => {
      if (t.startsWith(h2[0])) {
        ...
        return "number" == typeof t2 && (r3 = t2), true;
      }
      return false;
    }), r3;
  }
  ...
  _checkSizeLimits(t) {
    ...
    for (; s2; ) {
      s2 = false;
      for (const [r3, h2] of this._db) if (0 === h2.lifetime && (!i2 || r3.startsWith(i2))) {
        if (this._purgeItem(r3, h2, t), e2.size <= 0.9 * e2.maxSize) return;
        s2 || (s2 = this._db.has(r3));
      }
    }
    for (const [r3, h2] of this._db) if ((!i2 || r3.startsWith(i2)) && (this._purgeItem(r3, h2, t), e2.size <= 0.9 * e2.maxSize)) return;
  }
  _purgeItem(t, e2, i2 = this._users.find((e3) => t.startsWith(e3.id))) {
    if (this._db.delete(t), e2.lives <= 1) {
      ...
      const r3 = this._notifyRemove(t, e2.entry, e2.size, s.SOME);
      null != r3 && r3 > 0 && (this._size += r3, i2 && (i2.size += r3), e2.lives = e2.lifetime, e2.size = r3, this._db.set(t, e2));
...
//# sourceMappingURL=chunk-GKJXNCBF.js.map
