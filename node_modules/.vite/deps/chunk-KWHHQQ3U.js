...
// node_modules/@arcgis/core-adapter/dist/index.js
...
async function importCoreReactiveUtils() {
  ...
  const module = await import("./@arcgis_core_core_reactiveUtils__js.js");
...
var AccessorController = class extends GenericController {
  ...
  async hostLoad() {
    ...
    const finalParameters = ((_b = (_a = this._options) == null ? void 0 : _a.editConstructorProperties) == null ? void 0 : _b.call(component, parameters)) ?? parameters;
  ...
  _gatherParameters() {
    ...
    const props = isEsriInternalEnv() ? (filterWatchedProperties == null ? void 0 : filterWatchedProperties(this, data)) ?? data : data;
    return props;
  }
...
var accessorSupport = {
  makeGetterProxy: (component, watchedProperties, isBinding, allowedPropNameMismatches) => new Proxy(
    {},
    {
      ...
      get: (target, prop) => {
        ...
        const doBinding = (isBinding == null ? void 0 : isBinding.value) ?? true;
        if (hasProp || !doBinding) {
          return value;
        }
        if (watchedProperties.has(prop)) {
          if (isEsriInternalEnv() && !component.manager.isLit) {
            throw new Error(
        ...
        return trackKey(
          ...
          (resolved) => {
            if (isEsriInternalEnv()) {
              ...
              const collidesWithCustomProp = (allowedPropNameMismatches == null ? void 0 : allowedPropNameMismatches.has(prop)) === true;
              if (resolved.key !== prop && !collidesWithNativeProp && !collidesWithCustomProp && !isFlippingBoolean) {
                throw new Error(
                  `Tried to bind "${resolved == null ? void 0 : resolved.key}" property to "${prop.toString()}" - property names must match`
                );
              }
  ...
  watchAccessorUpdates(controller, instance, watchedProperties) {
    ...
    if (isEsriInternalEnv()) {
      setReadonlyProps == null ? void 0 : setReadonlyProps(controller, readonlyProps);
    }
    bypassGetter(
      () => (
        // Careful: Map's forEach callback arguments are (value, key), not (key, value)
        watchedProperties.forEach((propertyName, propName) => {
          if (isEsriInternalEnv() && !(propertyName in instance)) {
...
function findReadOnlyAccessorProps(instance) {
  ...
  const properties = Object.entries(((_a = accessor.__accessor__) == null ? void 0 : _a.metadata) ?? {});
  return new Set(
    properties.filter(([_property, descriptor]) => (descriptor == null ? void 0 : descriptor.readOnly) === true).map(([property]) => property)
  );
}
var setReadonlyProps = isEsriInternalEnv() ? (controller, properties) => {
  var _a;
  (_a = devOnlySetPersistentControllerData) == null ? void 0 : _a(controller, properties);
} : void 0;
var filterWatchedProperties = isEsriInternalEnv() ? (controller, data) => {
  var _a;
  const readonlyProperties = (_a = devOnlyGetPersistentControllerData) == null ? void 0 : _a(controller);
...
//# sourceMappingURL=chunk-KWHHQQ3U.js.map
