...
function f({ predicate: o = () => true, snappingManager: i, snappingContext: l2, updatingHandles: u, useZ: c = true }) {
  const p = new w();
  if (null == i) return { snappingStep: [y, p], cancelSnapping: y };
  let f2, Z = null, j2 = null, z = null;
  const T = () => {
    Z = e(Z), i.doneSnapping(), j2 == null ? void 0 : j2.frameTask.remove(), j2 = null, f2 = l(f2), z = null;
  }, k2 = d3(i, c, p);
  let w2 = null, I = null, U = null;
  return { snappingStep: [(n) => {
    if (!o(n)) return n;
    const { action: e3 } = n;
    if ("start" === e3) {
      const { info: e4 } = n, t = m(i.view);
      if (j2 = g2(l2, n, t), j2.context.selfSnappingZ = null, !c && null != e4) {
        const n2 = S(l2.coordinateHelper, e4.handle.component);
        null != n2 && (j2.context.selfSnappingZ = { value: n2, elevationInfo: l2.elevationInfo ?? j });
      }
    }
    if (null != j2) {
      const { context: o2, originalScenePos: l3, originalPos: a2 } = j2, { mapEnd: s, mapStart: p2, scenePoints: d4 } = n, m2 = x2(a2, v(s, p2)), g3 = v(p2, a2), S2 = { ...n, action: "update" }, y2 = j2.context, T2 = P(l3, d4), C = i.update({ point: m2, scenePoint: T2, context: o2 });
      if (U = C, h(s, C, g3, c), w2 = m2, I = T2, "end" !== e3) {
        const { frameTask: n2 } = j2;
        null == Z && (Z = new AbortController()), z = (e4) => {
          u.addPromise(d(k2({ frameTask: n2, event: S2, context: y2, point: m2, scenePoint: T2, delta: g3, getLastState: () => ({ point: w2, scenePoint: I, updatePoint: e4.forceUpdate ? null : U }) }, Z.signal)));
...
function m(n) {
  return "3d" === n.type ? n.resourceController.scheduler.registerTask(g.SNAPPING) : F;
}
function g2(n, e3, t) {
...
//# sourceMappingURL=chunk-XEZLBT7S.js.map
