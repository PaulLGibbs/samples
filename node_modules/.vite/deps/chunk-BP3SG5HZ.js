...
function x() {
  const x2 = new i();
  x2.include(e), x2.fragment.uniforms.add(new s("edgesTexture", (e2) => e2.inputTexture), new s("areaTexture", (e2) => e2.areaTexture), new s("searchTexture", (e2) => e2.searchTexture)), x2.fragment.constants.add("smaaAreaTexPixelSize", "vec2", [1 / 160, 1 / 560]);
  const d2 = 1 / 7;
  return x2.fragment.code.add(n`
    ...
    vec2 getArea(sampler2D areaTex, vec2 dist, float e1, float e2, float offset) {
      vec2 texcoord = float(${n.int(c)}) * round(4.0 * vec2(e1, e2)) + dist;
      texcoord = smaaAreaTexPixelSize * texcoord + (0.5 * smaaAreaTexPixelSize);
      texcoord.y += ${n.float(d2)} * offset;
      return texture(areaTex, texcoord).rg;
    }`), x2.fragment.main.add(n`
    ...
    vec4 maxOffset = vec4(offsets[0].xz, offsets[1].yw) + vec4(-2.0, 2.0, -2.0, 2.0) * resolution.xxyy * float(${n.int(s2)});
...
//# sourceMappingURL=chunk-BP3SG5HZ.js.map
